  ...
  get coords() {
    ...
    if (this.p1.x === this.p2.x) {
      ...
      if (this.p1.x > this.p2.x) {
        ...
        } else if (this.p1.x === this.p1.y && this.p2.x === this.p2.y) {
      const min = Math.min(this.p1.x, this.p2.x)
      const max = Math.max(this.p1.x, this.p2.x)
      for (let i = min; i <= max; i++) {
        coords.push({ x: i, y: i})
      }
    } else if (this.p1.x === this.p2.y && this.p1.y === this.p2.x) {
      const max = Math.max(this.p1.x, this.p1.y); // 3
      const min = Math.min(this.p1.x, this.p1.y); // 1
      for (let i = 0; i <= max-min; i++) {
        coords.push({ x: min + i, y: max - i})
      }
    }
    else {
